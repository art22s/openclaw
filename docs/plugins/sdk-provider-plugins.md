...
preparation callback. The separate synchronous `beforeModelAccessWrite` callback
runs immediately before the post-save policy commit; use it for any additional
current config-writer authority check, and throw to reject the write.
...
current config. The original
`beforeModelAccessWrite` callback is not retained. The original login callback does
not authorize a later command. Let the shared owner report the visibility outcome:
